...
public class CarEdit extends Frame implements ActionListener {
	 ...
	 */
	public CarEdit(byte[] img, String cname, String type, String fuel, String people, String bag, String release) {
		setFrame("차량수정", 600, 300);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(regist)) {
			if (pictureRegist || textField.getText().equals("제목을 입력해주세요.") || ts[0].getText().equals("탑승 가능한 최대 인원을 입력해주세요.") || ts[1].getText().equals("적재가능한 가방의 최대 개수를 입력해주세요.") || ts[2].getText().equals("등록할 차량의 출시일을 입력해주세요.")) {
				...
				ok("등록이 완료되었습니다.");
				DB.update("update car set c_name = ?, t_no = ?, f_no = ?, c_people = ?, c_release = ?, c_space = ?, c_img = ?", textField.getText(), c1.getSelectedIndex() + 1, c2.getSelectedIndex() + 1, ts[0].getText(), ts[2].getText(), ts[1].getText(), getByte(String.valueOf(path)));
			}
		}
	}
